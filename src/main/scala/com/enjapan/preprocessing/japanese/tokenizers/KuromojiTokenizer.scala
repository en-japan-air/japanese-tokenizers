...
/**
  ...
  */
class KuromojiTokenizer(tokenizer:Tokenizer = new Tokenizer(), stopWords: Set[String] = Set.empty[String], whiteListPOS: Set[List[String]] = Set.empty[List[String]], stopPOS: Set[List[String]] = Set.empty[List[String]]) {
  ...
  def tokenize(string: String): Seq[String] = {
    ...
    tokens
      ...
      .flatMap { t =>
          Option(t.getBaseForm).filterNot(_.contains("*")).filterNot(_.trim.isEmpty)
